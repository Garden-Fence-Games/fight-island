...
**Unreleased entries are not in this file.** They are one file per change under
[`changelog.d/`](changelog.d/), assembled here at release time by
`tools/assemble-changelog.sh`. Write yours there.
...
- **A body is as tough on wave fifteen as on wave one; the crowd grows instead.** Health no longer
  scales with the waves, and the pirate is three farmhands of it, always — so a blow learned on wave
  one kills the same man on wave fifteen and every weapon upgrade is pure gain. The waves are harder
  because there are more of them: a roster of 20 on wave 1 rising to 57 on wave 15, and up to 28
  standing at once, where it was 17 to 34 and 16. Damage still rises with the waves as before.
...
- **The big glowing farmer is a runner, not a harder fight.** It stands where it arrived, notices
  the player from closer than anyone, and then runs away from them — slower than the player walks,
  so it can always be caught — and it never swings. Catch it and it throws 20 coins and 15 rounds.
  Reach the sea and it slips under with all of it; the end of the wave takes it too. Exactly one
  comes on wave 3, and from wave 4 each body has a flat one-in-ten chance of being one.
...
- **The merchant sells one more upgrade every three waves.** One purchase after waves 1 to 3, two
  after 4 to 6, and on up to five by the last three — the same track may be bought more than once
  in a visit, and the screen stays open until the allowance is spent. The bigger late crowds pay
  more, and this is where that money goes. A saved run keeps count of what was already bought.
...
[1.0.0]: https://github.com/pepito2t/fight-island/compare/v0.2.0...v1.0.0
