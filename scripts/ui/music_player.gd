...
## The soundtrack in the corner: what is playing, a switch to silence it, and a button for the next
## one.
##
...
## What the buttons say. Glyphs rather than words: the row is small, and a speaker with a line
## through it needs no translating.
const MUTED_GLYPH: String = "🔇"
const SOUND_GLYPH: String = "🔊"
const NEXT_GLYPH: String = "⏭"
...
func _refresh() -> void:
	...
	if box == null or not box.has_music():
		...
		return
	visible = true
	mute.text = MUTED_GLYPH if box.is_muted() else SOUND_GLYPH
